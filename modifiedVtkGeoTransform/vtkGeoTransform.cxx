...
//double srcToRadians = 0.0;
//double destToRadians = 0.0;
//double RAD_TO_DEG = 180/cmath.pi;
//bool srcIsLongLat = false;
//bool destIsLongLat = false;
...
vtkGeoTransform::vtkGeoTransform()
{
  ...
  //this->srcToRadians = 0.0;
  //this->destToRadians = 0.0;
  this->srcToRadians = 180/M_PI;
  this->destToRadians = 180/M_PI;
  this->RAD_TO_DEG = 180/M_PI;
  //this->srcIsLongLat = false;
  this->srcIsLongLat = true;
  this->destIsLongLat = false;
...
bool vtkGeoTransform::GetSrcIsLongLat()
{
  return this->srcIsLongLat;
}

void vtkGeoTransform::SetSrcIsLongLat(bool val)
{
  this->srcIsLongLat = val;
}

bool vtkGeoTransform::GetDestIsLongLat()
{
  return this->destIsLongLat;
}

void vtkGeoTransform::SetDestIsLongLat(bool val)
{
  this->destIsLongLat = val;
}
...
/* we want to change the function to take a list of points */
/* and return a list of points ordered like: */
/* x_0, y_0, z_0, ... , x_n, y_n, z_n                                    */
/************************************************************************/
vtkPoints* vtkGeoTransform::process(float *pointlist, int numPoints)

{

    PJ_UV data;
    vtkPoints *pointsout = vtkPoints::New();
    float *firstPoint = pointlist;
    ...
    transformation = proj_create_crs_to_crs_from_pj(nullptr, SourceProjection->GetProjection(), DestinationProjection->GetProjection(), pj_area, options.data());

    //while (WESTILLHAVEPOINTSTOPROCESS)
    while (pointlist < firstPoint + (numPoints * 3))
    {
        double z, u, v;
        ...
        if (data.u != HUGE_VAL) {
            if (this->srcIsLongLat) {
                /* dmstor gives values to radians. Convert now to the SRS unit
                 */
                std::cout << "srcIsLongLat" << std::endl;
	        std::cout << "data.u before conversion to radians: " << data.u << std::endl;
	        std::cout << "data.v before conversion to radians: " << data.v << std::endl;
                data.u /= srcToRadians;
                data.v /= srcToRadians;
            }
	    ...
	    std::cout << "data.u before transformation: " << data.u << std::endl;
	    std::cout << "data.v before transformation: " << data.v << std::endl;
	    std::cout << "z before transformation: " << z << std::endl;
	    ...
	    std::cout << "data.u: " << data.u << std::endl;
	    std::cout << "data.v: " << data.v << std::endl;
        ...
        if (data.u == HUGE_VAL){ /* error output */
            //fputs(oterr, stdout);
            std::cout << "IS HUGE_VAL" << std::endl;
        }

        else if (this->destIsLongLat && !this->oform) { /*ascii DMS output */

            fputs("trying to print degrees, minutes, seconds, shame on you", stderr);

        } else { /* x-y or decimal degree acii output */
            if (this->destIsLongLat) {
                data.v *= destToRadians * RAD_TO_DEG;
                data.u *= destToRadians *RAD_TO_DEG;
                //pointsout.InsertNextPoint(data.u, data.v, data.z)
                pointsout->InsertNextPoint(data.u, data.v, z);
            }
        }
        pointlist = pointlist + 3; // Move the pointer 3 indices over to get the next coordinate
    }
    return pointsout;
}
VTK_ABI_NAMESPACE_END
